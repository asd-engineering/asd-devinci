...
DEFAULT_TUNNEL_HOST="cicd.eu1.asd.engineer"
DEFAULT_TUNNEL_PORT="2223"
...
if [ -n "${INPUT_CLIENT_ID:-}" ] && [ -n "${INPUT_CLIENT_SECRET:-}" ]; then
  ...
  # Use explicit inputs or defaults for tunnel host/port
  ASD_TUNNEL_HOST="${ASD_TUNNEL_HOST:-$DEFAULT_TUNNEL_HOST}"
  ASD_TUNNEL_PORT="${ASD_TUNNEL_PORT:-$DEFAULT_TUNNEL_PORT}"

  echo "Using pre-existing credentials: client_id=${ASD_CLIENT_ID}"

else
  # === EPHEMERAL TOKEN MODE ===
  # Creates short-lived tunnel credentials via create-ephemeral-token edge function
  # Works with or without an API key
  echo "Creating ephemeral tunnel credentials via API..."
  ...
  # Read tunnel host/port from API response, fall back to defaults
  TUNNEL_HOST_FROM_API=$(echo "$RESPONSE" | jq -r '.tunnel_host // empty')
  TUNNEL_PORT_FROM_API=$(echo "$RESPONSE" | jq -r '.tunnel_port // empty')

  # Priority: explicit input > API response > default
  if [ -n "${ASD_TUNNEL_HOST:-}" ]; then
    : # keep explicit input
  elif [ -n "$TUNNEL_HOST_FROM_API" ]; then
    ASD_TUNNEL_HOST="$TUNNEL_HOST_FROM_API"
  else
    ASD_TUNNEL_HOST="$DEFAULT_TUNNEL_HOST"
  fi

  if [ -n "${ASD_TUNNEL_PORT:-}" ]; then
    : # keep explicit input
  elif [ -n "$TUNNEL_PORT_FROM_API" ]; then
    ASD_TUNNEL_PORT="$TUNNEL_PORT_FROM_API"
  else
    ASD_TUNNEL_PORT="$DEFAULT_TUNNEL_PORT"
  fi

  echo "Provisioned: tunnel_user=${ASD_CLIENT_ID}, expires=${EXPIRES_AT}"
fi
...
if [ -n "${ASD_API_KEY:-}" ]; then
  ...
  echo "API key exported for in-session ASD CLI use"
fi
