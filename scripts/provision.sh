...
# Set tunnel host/port if not already set by ephemeral response
if ! grep -q "ASD_TUNNEL_HOST=" "$GITHUB_ENV" 2>/dev/null; then
  echo "ASD_TUNNEL_HOST=${ASD_TUNNEL_HOST:-cicd.eu1.asd.engineer}" >> "$GITHUB_ENV"
fi
if ! grep -q "ASD_TUNNEL_PORT=" "$GITHUB_ENV" 2>/dev/null; then
  echo "ASD_TUNNEL_PORT=${ASD_TUNNEL_PORT:-2223}" >> "$GITHUB_ENV"
fi
